...
```{r, echo=FALSE}
source("funciones.R") #funciones auxiliares prediseñadas por el profesor
```
...
student_scores <- read.csv("DataSets/student-scores.csv")
summary(student_scores) #Estadística descriptiva de los datos numéricos
```
...
```{r,echo=TRUE,message=FALSE,warning=FALSE}
vars_predict <- c('math_score','history_score','physics_score',"chemistry_score",
                  "biology_score","english_score","geography_score")  #covariables
...
```{r,echo=TRUE,message=FALSE,warning=FALSE, fig.height=6, fig.width=6}
par(mfrow = c(2, 2))
...
```{r,echo=TRUE,message=FALSE,warning=FALSE, fig.height=7, fig.width=6}
biplot(comps)
```
